...
public class EntityAfterImageWeaponParticle extends CustomModelParticle<AnimatedMesh> {
	...
	public EntityAfterImageWeaponParticle(ClientLevel level, double x, double y, double z, double xd, double yd, double zd, AnimatedMesh particleMesh, OpenMatrix4f[] matrices, Matrix4f modelMatrix) {
	...
	public static class Provider implements ParticleProvider<SimpleParticleType> {
		...
		public Particle createParticle(SimpleParticleType typeIn, ClientLevel level, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
			...
			if (entitypatch != null && ClientEngine.getInstance().renderEngine.hasRendererFor(entitypatch.getOriginal())) {
				...
				renderer.mulPoseStack(poseStack, armature, entitypatch.getOriginal(), entitypatch, 6.0F);
